...
proc = subprocess.Popen(
    cmd,
    stdout=open(job_folder / "stdout.log", "w"),
    stderr=open(job_folder / "stderr.log", "w"),
)
...
result pickle on NFS. Implements the full `PrefectFuture` protocol including
callbacks:
...
subprocesses at once is problematic. A `BoundedSemaphore` caps the number of
concurrently launching srun processes. Note that this limits the *launch
rate*, not the number running on SLURM — once launched, the srun process
blocks inside SLURM until resources are available:

```python
DEFAULT_SRUN_LAUNCH_CONCURRENCY = 128

# In srun mode, map launches concurrent srun processes with a launch limiter
semaphore = threading.BoundedSemaphore(DEFAULT_SRUN_LAUNCH_CONCURRENCY)
futures = []
for i, wrapped_call in enumerate(wrapped_calls):
    ...
    self._srun_procs.append(proc)
    future = SrunPrefectFuture(proc, job_folder, task_run_ids[i], ...)
    # Release semaphore slot when this task completes
...
semaphore prevents the orchestrator from accumulating thousands of blocked
subprocesses.
...
**Concurrency control.** **Decision:** use a `BoundedSemaphore` (default
128) in the map path to cap concurrently launching subprocesses. See the
map section above.
...
- `tests/test_futures_srun.py` — unit tests for `SrunPrefectFuture`
